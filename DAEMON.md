...
Mechanics — **read-only by default**: without an explicit `--edit-token`
...
`daemon/` are gone. The cron scheduler now runs daemon-side — the daemon
...
- ✅ **Audit log**: `--audit-log <path>` appends JSONL — connects (peer
  ...
  logged with `allowed:false` — those are the interesting entries.
...
The frontend is **embedded in the binary** (include_dir) — a release
